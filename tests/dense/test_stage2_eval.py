...
def test_perfect_prediction_scores_iou_one():
    ...
    act_size = cat[gt_assign]  # ascending per-axis == visible axis order below
    vis_size = act_size.clone()  # so argsort places the SKU dims correctly
    vis_center = torch.tensor([[0.1, 0.0, 1.0], [0.2, 0.1, 1.5]])
    act_center = torch.tensor([[0.1, 0.0, 1.2], [0.2, 0.1, 1.8]])
    ident6 = torch.tensor([[1.0, 0, 0, 0, 1, 0]] * 2)  # identity rotation, shared
